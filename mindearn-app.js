(function () {
  const settings = window.MindEarnSettings || {
    analyticsEndpoint: "",
    subscribeEndpoint: "",
    leadEmail: "sandor@innoviaworld.hu",
    access: {
      book: "active",
      workbook: "active",
      course: "building",
  ...
  function wireSubscriptionForms() {
    document.querySelectorAll("[data-subscribe-form]").forEach((form) => {
      ...
      form.addEventListener("submit", async (event) => {
        ...
        if (settings.subscribeEndpoint) {
          try {
            const response = await fetch(settings.subscribeEndpoint, {
              method: "POST",
        ...
        if (status) {
          status.innerHTML = `Az űrlap készen áll. <a href="mailto:${settings.leadEmail}?subject=${subject}&body=${body}">Email küldése</a>`;
        }
      });
    });
  }
  ...
  function applyAccessState() {
    document.querySelectorAll("[data-access-key]").forEach((item) => {
      ...
      const state = settings.access[key] || "waiting";
